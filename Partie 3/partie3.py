...
class Player_AI():
    ...
    def makeMove(self, board, G):
        return self.makeThinkingMove(listMoves(board, self.color, self.N, self.WALLS, G), board, self.color, self.NN, self.eps, self.learning_strategy)
    ...
    def makeThinkingMove(self,moves, s, color, NN, eps, learning_strategy=None):
        ...
        if Q_learning or TD_lambda:
            ...
            copy = (NN[0].copy(),NN[1].copy())
            self.NN = backpropagation(s, NN, delta, learning_strategy)
